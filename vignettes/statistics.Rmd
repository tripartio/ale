...
We can see that most variables seem to have some sort of mean effect across various values. However, our focus must be on the bootstrap intervals. Crucial to our interpretation is the middle grey bar that indicates the median ± 2.5%, that is, the middle 5% of all average mathematics achievement scores (`math_avg`) values in the dataset. The idea is that if any predictor can do no better than influencing `math_avg` to fall within this middle bar, then it only has a minimal effect. For an effect to be considered statistically significant, there should be no overlap between the confidence regions of a predictor variable and the middle bar.
...
However, although ALE plots allow rapid and intuitive conclusions, it is often helpful to have numbers that give us these conclusions. Thus, we have developed a collection of effect size measures based on ALE tailored for intuitive interpretation.
...
With this understanding, let us explain the ALE effect size measures. They are accessible through the `ale$stats` element of the bootstrap result object, with multiple views. To focus on all the measures for a specific variable, we can access the `ale$stats$by_term` element. Here are the effect size measures for the categorical `public`:

```{r ALE stats for public}
mb_gam$ale$stats$by_term$public
```

Let's look at the plot for `public` to help us understand what these measures tell us.
...
We can see that the same ALE measures apply to categorical and numeric variables, even though their plots are quite different, as we see here:

```{r ALE plot for academic_ratio}
mb_gam$ale$plots$academic_ratio
```

Before we present the various ALE effect size measures, we must reiterate the timeless reminder that correlation is not causation. So, none of the scores necessarily means that an x variable *causes* a certain effect on the y outcome; we can only say that the ALE effect size measures indicate associated or related variations between the two variables. 
...
The transformed maximum ALE y is thus scaled as a percentile from 0 to 100%. Its formula is

$$
\mathrm{norm\_ale\_y} = 100 \begin{cases} 
...
$$
\mathrm{NALED}(y, \mathrm{ale\_y}, \mathrm{ale\_n}) = \frac{\mathrm{ALED}(\mathrm{norm\_ale\_y}, \mathrm{ale\_n})}{2}
$$

NALED produces a score that ranges from 0 to 100%. It is essentially the ALED expressed in percentiles, that is, the average effect of a variable over its full domain of values. So, the NALED of public school status of `r c(mb_gam$ale$stats$by_term$public$estimate['naled']) |> round(2)` indicates that its average effect on math scores spans the middle `r mb_gam$ale$stats$by_term$public$estimate['naled'] |> round()` percent of scores. Academic ratio has an average effect expressed in NALED of `r c(mb_gam$ale$stats$by_term$academic_ratio$estimate['naled']) |> round(1)`% of scores.
