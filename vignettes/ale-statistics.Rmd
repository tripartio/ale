...
In this vignette, we cannot go into the details of how GAM models work (you can learn more with [Noam Ross's excellent tutorial](https://noamross.github.io/gams-in-r-course/chapter1/ "Tutorial on GAM")), but we note that for our model illustration, only the details of the non-numeric variables are shown here. The numeric variables are handled with GAM smoothing functions, so they do not have regular regression coefficients.
...
We can see that most variables seem to have some sort of mean effect across various values. However, our focus must be on the bootstrap intervals. Crucial to our interpretation is the middle grey bar that indicates the median ± 2.5%, that is, the middle 5% of all average mathematics achievement scores (`math_avg`) values in the dataset. The idea is that if any predictor can do no better than influencing `math_avg` to fall within this middle bar, then it only has a minimal effect. Thus, we may call this the "middle bar of irrelevance". For an effect to be considered statistically significant, there should be no overlap between the confidence regions of a predictor variable and the middle bar. (We use 5% by default, but the value can be changed with the `median_bar` argument.)
...
To understand how ALE effect size measures are calculated, we must understand the structure of ALE data. Let's begin simple with a binary variable with just two categories, `public`:
...
In a bootstrapped ALE plot, values within the confidence intervals are statistically significant; values outside of the middle bar can be considered at least somewhat meaningful. Thus, **the essence of ALE-based statistical inference is that only effects that are simultaneously within the confidence intervals AND outside of the middle bar of irrelevance should be considered conceptually meaningful.**
...
-   **`relative_to_mid`** is the key information here. It indicates if all the values in sequence from `start_x` to `end_x` are below, overlapping, or above the middle bar of irrelevance:
    -   below: the higher limit of the confidence interval of ALE y (`ale_y_hi`) is below the lower limit of the middle bar. 
    -   above: the lower limit of the confidence interval of ALE y (`ale_y_lo`) is above the higher limit of the middle bar.
    -   overlap: neither of the first two conditions holds; that is, the confidence region from `ale_y_lo` to `ale_y_hi` at least partially overlaps the middle bar.

These results tell us simply that, for `academic_ratio`, from 0 to 0, ALE is below the middle bar from 9.22 to 9.22. From 0.05 to 0.05, ALE overlaps the middle bar from 10.9 to 10.9. From 0.09 to 0.09, ALE is below the middle bar from 11.7 to 11.7. From 0.1 to 0.91, ALE overlaps the middle bar from 12 to 13.9. From 0.95 to 1, ALE is above the middle bar from 14.2 to 14.7.

Interestingly, the text of the previous paragraph was generated automatically by an internal (unexported function) `ale:::summarize_conf_regions_in_words`. (Since the function is not exported, you must use `ale:::` with three hyphens, not just two, if you want to access it.)

```{r text conf_regions for academic_ratio}
ale:::summarize_conf_regions_in_words(mb_gam$ale$conf_regions$academic_ratio)
```

While the wording is rather mechanical, it immediately illustrates the value of being able to summarize the inferentially relevant conclusions in tabular form.
...
Since we have categories here, there is no start or end positions and there is no trend. We instead have each `x` category and its single ALE `y` value, with the `n` and `n_pct` of the respective category and `relative_to_mid` as before to indicate whether the indicated category is below, overlaps with, or is above the middle bar.

Again with the help of `ale:::summarize_conf_regions_in_words`, these results tell us that, for `public`, for FALSE, the ALE of 13.4 overlaps the middle bar. For TRUE, the ALE of 12.6 overlaps the middle bar.

## ALE effect size measures

Although ALE plots allow rapid and intuitive conclusions for statistical inference, it is often helpful to have summary numbers that quantify the average strengths of the effects of a variable. Thus, we have developed a collection of effect size measures based on ALE tailored for intuitive interpretation. They are accessible through the `ale$stats` element of the bootstrap result object, with multiple views. To focus on all the measures for a specific variable, we can access the `ale$stats$by_term` element. Here are the effect size measures for the categorical `public`:
...
* The middle bar represents the median ± 2.5% of values.
