...
As you can imagine, this procedure is very slow: it involves retraining the entire model on the full dataset 1,000 times. The `ale` package can speed up the process significantly through parallel processing, but it still involves the speed of retraining the model hundreds of times. 
...
```{r ALEpDist, eval=FALSE, echo=TRUE}
...
```{r ALEpDist-rds, eval=TRUE, echo=FALSE}
...
```{r model bootstrap-rds, eval=TRUE, echo=FALSE}
...
```{r model-bootstrap-without-p, eval=FALSE, echo=TRUE, fig.width=7, fig.height=10}
# # To run the slow code yourself, uncomment and execute this code block directly.
# For models like mgcv::gam that store their data,
# there is no need to specify the data argument.
...
```{r model-bootstrap-without-p-rds, eval=TRUE, echo=FALSE, fig.width=7, fig.height=10}
...
-   **Normalized ALE deviation (NALED):** this is the average variation of ALE of the input variable.
