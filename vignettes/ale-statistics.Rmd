...
Accumulated local effects (ALE) was developed by Daniel Apley and Jingyu Zhu <doi:10.1111/rssb.12377> as a global explanation approach for interpretable machine learning (IML). However, the [`ale` package](ale-intro.html) aims to extend it for statistical inference, among other extensions. This vignette presents the initial effort at extending ALE for statistical inference. In particular, we present some effect size measures specific to ALE. We introduce these statistics in detail in a working paper: Okoli, Chitu. 2023. "Statistical Inference Using Machine Learning and Classical Techniques Based on Accumulated Local Effects (ALE)." arXiv. <https://doi.org/10.48550/arXiv.2310.09877>. Please note that they might be further refined after peer review.
...
-   `median` and `mean` are the average bootstrapped estimates of the statistic.
...
We see there that `public` has an ALER of [`r c(get(mb_gam_math, 'public', stats = 'all')$estimate['aler_min'], get(mb_gam_math, 'public', stats = 'all')$estimate['aler_max']) |> round(2)`]. When we consider that the median math score in the dataset is `r median(math$math_avg) |> round(1)`, this ALER indicates that the minimum of any ALE y value for `public` (when `public == TRUE`) is `r get(mb_gam_math, 'public', stats = 'all')$estimate['aler_min'] |> round(2)` below the median. This is shown at the `r (median(math$math_avg) + get(mb_gam_math, 'public', stats = 'all')$estimate['aler_min']) |> round(1)` mark in the plot above. The maximum (`public == FALSE`) is `r get(mb_gam_math, 'public', stats = 'all')$estimate['aler_max'] |> round(2)` above the median, shown at the `r (median(math$math_avg) + get(mb_gam_math, 'public', stats = 'all')$estimate['aler_max']) |> round(1)` point above.
...
The ALER for `academic_ratio` is considerably broader with `r get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['aler_min'] |> round(2)` below and `r get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['aler_max'] |> round(2)` above the median.
...
ALER shows the extreme values of a variable's effect on the outcome. In the effects plot above, it is indicated by the extreme ends of the horizontal bars for each variable. We see there that `public` has an ALER of `r c(get(mb_gam_math, 'public', stats = 'all')$estimate['aler_min'], get(mb_gam_math, 'public', stats = 'all')$estimate['aler_max']) |> round(2)`. When we consider that the median math score in the dataset is `r math$math_avg |> median() |> round(1)`, this ALER indicates that the minimum of any ALE y value for `public` (when `public == TRUE`) is `r get(mb_gam_math, 'public', stats = 'all')$estimate['aler_min'] |> round(2)` below the median. This is shown at the `r (math$math_avg |> median() + get(mb_gam_math, 'public', stats = 'all')$estimate['aler_min']) |> round(1)` mark in the plot above. The maximum (`public == FALSE`) is `r get(mb_gam_math, 'public', stats = 'all')$estimate['aler_max'] |> round(2)` above the median, shown at the `r (math$math_avg |> median() + get(mb_gam_math, 'public', stats = 'all')$estimate['aler_max']) |> round(1)` point above. The ALER for `academic_ratio` is considerably broader with `r get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['aler_min'] |> round(2)` below and `r get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['aler_max'] |> round(2)` above the median.
...
So, with a NALER of `r c(get(mb_gam_math, 'public', stats = 'all')$estimate['naler_min'], get(mb_gam_math, 'public', stats = 'all')$estimate['naler_max']) |> round(2)`, the minimum of any ALE value for `public` (`public == TRUE`) shifts math scores by `r get(mb_gam_math, 'public', stats = 'all')$estimate['naler_min'] |> round()` percentile y points whereas the maximum (`public == FALSE`) shifts math scores by `r get(mb_gam_math, 'public', stats = 'all')$estimate['naler_max'] |> round()` percentile points. Academic track ratio has a NALER of `r c(get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['naler_min'], get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['naler_max']) |> round(2)`, ranging from `r get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['naler_min'] |> round()` to `r get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['naler_max'] |> round()` percentile points of math scores.
...
NALED produces a score that ranges from 0 to 100%. It is essentially the ALED expressed in percentiles, that is, the average effect of a variable over its full domain of values. So, the NALED of public school status of `r get(mb_gam_math, 'public', stats = 'all')$estimate['naled'] |> round(1)` indicates that its average effect on math scores spans the middle `r get(mb_gam_math, 'public', stats = 'all')$estimate['naled'] |> round(1)` percent of scores. Academic ratio has an average effect expressed in NALED of `r get(mb_gam_math, 'academic_ratio', stats = 'all')$estimate['naled'] |> round(1)`% of scores.
...
get(mb_gam_math, 'mean_ses', stats = 'all')
```
...
These results tell us that, for `mean_ses`, from -1.19 to -1.04, ALE is below the ALER band from 6.1 to 7.6. From -0.792 to -0.792, ALE overlaps the ALER band from 10.2 to 10.2. From -0.756 to -0.674, ALE is below the ALER band from 10.2 to 10.8. From -0.663 to -0.663, ALE overlaps the ALER band from 10.9 to 10.9. From -0.643 to -0.484, ALE is below the ALER band from 10.8 to 11.2. From -0.467 to -0.467, ALE overlaps the ALER band from 11.5 to 11.5. From -0.46 to -0.46, ALE is below the ALER band from 11.4 to 11.4. A few other regions briefly exceeded the ALER band.
...
Despite any apparent pattern, we see that from -2.4 to 2.61, ALE overlaps the ALER band from 12 to 12.8. So, despite the random highs and lows in the bootstrap confidence interval, there is no reason to suppose that the random variable has any effect anywhere in its domain.
