...
```{r load libraries}
library(mgcv)   # for datasets and for the gam function
...
So, with a NALER of `r c(mb_gam$ale$stats$by_term$public$estimate['naler_min'], mb_gam$ale$stats$by_term$public$estimate['naler_max']) |> round(2)`, the minimum of any ALE value for `public` (`public == TRUE`) shifts math scores by `r mb_gam$ale$stats$by_term$public$estimate['naler_min'] |> round()` percentile y points whereas the maximum (`public == FALSE`) shifts math scores by `r mb_gam$ale$stats$by_term$public$estimate['naler_max'] |> round()` percentile points. Academic track ratio has a NALER of `r c(mb_gam$ale$stats$by_term$academic_ratio$estimate['naler_min'], mb_gam$ale$stats$by_term$academic_ratio$estimate['naler_max']) |> round(2)`, ranging from `r mb_gam$ale$stats$by_term$academic_ratio$estimate['naler_min'] |> round()` to `r mb_gam$ale$stats$by_term$academic_ratio$estimate['naler_max'] |> round()` percentile points of math scores.
...
-   **Normalized ALE deviation (NALED):** this is the average variation of ALE effect of the input variable.
    ...
    -   NALED \< 5% has the same average effect as a random variable. That is, the average effect is not reliable; it might be random.
        -   **NOTE:** we should consider the NALER and the ALE plot to see if there are some extreme values that do have a strong effect, even if the vast majority of values do not have a meaningful effect.
        -   This caveat is very important; unlike GLM coefficients, ALE analysis is sensitive to exceptions to the overall trend. This is precisely what makes it valuable for detecting non-linear effects.
...
In general, regardless of the values of ALE statistics, we should always visually inspect the ALE plots to identify and interpret patterns of relationships between inputs and the outcome. However, when we have many variables in an analysis or we have procedures that must make automated decisions based on the strength of ALE effects, it is helpful to consider some rules of thumb for what might generally be considered meaningful ALE effect size measures.

Regardless of the average effect indicated by NALED, large NALER effects indicate that the ALE plot should be inspected to interpret the exceptional cases. In general, if NALED \< 5%, and NALER minimum \> –5%, and NALER maximum \< +5%, the input variable has no meaningful effect. All other cases are worth inspecting the ALE plots for careful interpretation:
...
It might not always be easy to tell from a plot which regions are relevant, so the results of statistical significance are summarized with the `ale$conf_regions` element, which can be accessed for each variable from its `by_term` element:

```{r conf_regions for academic_ratio}
mb_gam$ale$conf_regions$by_term$academic_ratio
```
...
ale:::summarize_conf_regions_in_words(mb_gam$ale$conf_regions$by_term$academic_ratio)
```
...
mb_gam$ale$conf_regions$by_term$public
```
...
mb_gam$ale$conf_regions$by_term$rand_norm
```
...
mb_gam$ale$conf_regions$significant
```
...
mb_gam$ale$conf_regions$significant$term |> 
  unique()
```
