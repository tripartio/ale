...
Here is the analogous code using the `ale` package. In this case, we also need to define a custom predict function because of the particular `n.trees = 6000` argument. Again, to speed things up, we provide a pretrained model. 
...
In our view, log odds are not the most interpretable way to express probabilities. So, we repeat the ALE creation using the "response" prediction type for probabilities and the default median centring of the plots. 
...
gbm_ale_ixn_prob <- url('https://github.com/Tripartio/ale/raw/main/download/gbm_ale_ixn_probk.rds') |> 
  readRDS()
