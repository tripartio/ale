...
The `ale` package originally began as a rewrite and extension of the `ALEPlot`. Thus, in developing the `ale` package, we must ensure that we correctly implement the original ALE algorithm while extending it. Indeed, some permanent unit tests call the `ALEPlot` to make sure that `ale` provides identical results for identical inputs. We thought presenting some of these comparisons as a vignette might be helpful. We focus on the [examples from the `ALEPlot` package](https://cran.r-project.org/web/packages/ALEPlot/vignettes/AccumulatedLocalEffectPlot.pdf) so that results are directly comparable.
...
-   It saves plots as ggplot objects to a "plots" element of the return value; it does not automatically print the plot to the screen. So, the user can manipulate the plots more flexibly.
-   In the plot, the Y outcome variable is displayed by default on its full absolute scale, centred on the median or mean, not on a scale relative to zero. (This option can be controlled with the `relative_y` argument.) We such plots more easily interpretable.6
...
One notable difference between the two packages is that the `ale` package does not and will not implement partial dependency plots (PDP). The package is focused exclusively on accumulated local effects (ALE); users who need PDPs may use `ALEPlot`.
...
One point I dislike about the `ALEPlot` implementation is that calling the function automatically prints a plot, whether the user likes it or not. The user must jump through a few hoops to suspend graphic output to avoid this--the function does not give any such option.
...
nn_ale <- ale(DAT, nnet.DAT, predict_type = "raw")
```
...
* Unlike `ALEPlot` that functions on only one variable at a time, `ale` generates ALE data for multiple variables in a dataset at once. By default, it generates ALE elements for all the predictor variables in the dataset that it is given. We will cover them in another vignette, but for our purposes here, we note the `data` element that returns a list of the ALE data for each variable and the `plots` element returns a list of `ggplot` plots.
* `ale` creates a default generic predict function that matches most standard R models. When the prediction type is not the default "response", as in our case, the user can set the desired type with the `predict_type` argument. However, for more complex or non-standard prediction functions, `ale` supports custom functions with the `pred_fun` argument.
...
```{r}
# Zero-centred ALE
nn_ale <- ale(DAT, nnet.DAT, predict_type = "raw", relative_y = 'zero')
...
nn_ale_ixn <- ale_ixn(DAT, nnet.DAT, predict_type = "raw")
...
These interaction plots are heat maps that indicate the interaction regions that are above or below the average value of y with colours. Grey indicates no meaningful interaction. We find these easier to interpret than the contour maps from `ALEPlot`, especially since the colours in each plot are on the same scale and so the plots are directly comparable with each other.
...
Although gradient boosted trees generally perform quite well, they are rather slow. Rather than having you wait for it to run, the code here downloads a pretrained GBM model. However, the code used to generate it is provided in comments so that you can see it and run it yourself if you want to.
...
As before, we create a custom prediction function and then call the `ALEPlot` function to generate the plots. Creation of the ALE plots here is rather slow because the `gbm` predict function is slow.
...
Here is the analogous code using the `ale` package. In this case, we also need to define a custom predict function because of the particular `n.trees = 6000` argument. Again,  
...
# )
# saveRDS(gbm_ale, file.choose())
gbm_ale <- url('https://github.com/Tripartio/ale/raw/main/download/gbm_ale.rds') |> 
  readRDS()
...
# To generate the code, uncomment the following lines.
# But it is slow, so this vignette loads a pre-created model object.
# gbm_ale_ixn <- ale_ixn(data, gbm.data, pred_fun = yhat, x_intervals = 500)
