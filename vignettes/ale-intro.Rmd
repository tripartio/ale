...
A few entries are clearly invalid, where the length (x), width (y), or depth (z) is 0. In addition, around a quarter of the entries have unusual duplicate features. We will remove these for the analysis.
...
In this case, the bootstrapped results are mostly similar to single (non-bootstrapped) ALE result. In principle, we should always bootstrap the results and trust only in bootstrapped results. The most unusual result is that values of `x` (the length of the diamond) from 6.2 mm or so and higher are associated with lower diamond prices. When we compare this with the y value (height of the diamond), we suspect that when both the length and height (that is, the size) of a diamond become increasingly large, the price increases so much more rapidly with the height than with the length that the height (`y`) has an inordinately high effect that is tempered by a decreased effect of the length (`x`) at those high values. This would be worth further exploration for real analysis, but here we are just demonstrating the package.
...
Note that ALE interactions are very particular: an ALE interaction means that two variables have a composite effect over and above their separate independent effects. So, of course x and y both have effects on the price, as the one-way ALE plots show, but they have no additional composite effect. To see what ALE interaction plots look like in the presence of interactions, see the [ALEPlot comparison vignette](ale-ALEPlot.rmd), which explains the interaction plots in more detail.
