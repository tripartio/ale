...
A few entries are clearly invalid, where the length (x), width (y), or depth (z) is 0. We will remove these for the analysis.
...
Again, we provide here some demonstration code to plot all the ALE plots. It is a little more complex this time because of the two levels of interacting variables in the output data, so we use the `purrr` package to iterate the list structure.

```{r ale_ixn, fig.width=7, fig.height=7}
...
This is not the best dataset to use to illustrate ALE interactions because there are none here. This is expressed in the graphs by the ALE y values all falling in the middle grey band, which indicates that any interactions would not shift the price outside the middle 5% of its values. In other words, there is no meaningful interaction effect. 
...
[ALEPlot comparison vignette](ALEPlot.rmd), which explains the plots in more detail.
