...
nn_ale <- ALE(nnet.DAT, data = DAT, pred_type = "raw")
```
...
nn_1D_plots <- nn_plots@distinct$y$plots$d1
...
nn_1D_plots_zero <- nn_plots_zero@distinct$y$plots$d1
...
```{r ale nnet 2D, fig.width=7, fig.height=5}
...
nn_2D_plots <- nn_plots@distinct$y$plots$d2
nn_2D_plots |>
...
gbm_1D_link_plots <- gbm_link_plots@distinct$higher_income$plots$d1
...
```{r ale 2D link, fig.width=7, fig.height=5}
# # To generate the code, uncomment the following lines.
# # But it is very slow because it calculates ALE for all variables,
# # so this vignette loads a pre-created model object.
# gbm_ale_2D_link <- ALE(
...
gbm_link_2D_plots <- gbm_link_plots@distinct$higher_income$plots$d2
gbm_link_2D_plots |>
  # extract list of x1 ALE outputs
  purrr::walk(\(it.x1) {  
    # plot all x2 plots in each .x1 element
...
gbm_1D_prob_plots <- gbm_prob_plots@distinct$higher_income$plots$d1
...
```{r ale 2D prob, fig.width=7, fig.height=5}
# To generate the code, uncomment the following lines.
# # But it is slow because it calculates ALE for all variables,
# # so this vignette loads a pre-created model object.
# gbm_ale_2D_prob <- ALE(
...
gbm_prob_2D_plots <- gbm_prob_plots@distinct$higher_income$plots$d2
gbm_prob_2D_plots |>
  # extract list of x1 ALE outputs
  purrr::walk(\(it.x1) {  
    # plot all x2 plots in each .x1 element
