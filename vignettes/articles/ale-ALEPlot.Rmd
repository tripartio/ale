...
The [`{ALEPlot}` package](https://CRAN.r-project.org/package=ALEPlot) is the reference implementation of the brilliant idea of accumulated local effects (ALE) by Daniel Apley and Jingyu Zhu <doi:10.1111/rssb.12377>. However, it has not been updated since 2018. The [`{ale}` package](ale-intro.html "Introduction to the ale package") attempts to rewrite and extend the original base work.
...
-   In the plot, the Y outcome variable is displayed by default on its full absolute scale, centred on the median or mean, not on a scale relative to zero. (This option can be controlled with the `ale_centre` argument to [plot()], as we demonstrate.) We believe that such plots more easily interpretable.
...
-   By default, plots are centred on the median, which is more intuitive than zero-centring because it is easier to see the effects of a variable relative to typical values.
...
The `{ale}` also produces interaction plots; see the [introductory vignette](https://tripartio.github.io/ale/articles/ale-intro.html) for details on how they are specified and created. 
...
The range of outcome (y) values is divided into quantiles (deciles by default). However, the middle quantiles are modified. Rather than showing the middle 10% or 20% of values, it is much narrow: it shows the middle 5%. (This value is based on the notion of alpha of 0.05 for confidence intervals; it can be customized with the `y_nonsig_band` argument.)
...
ALE.1=ALEPlot(data[,-c(3,4,15)], gbm.data, pred.fun=yhat, J=1, K=100,
NA.plot = TRUE)
ALE.3=ALEPlot(data[,-c(3,4,15)], gbm.data, pred.fun=yhat, J=3, K=100,
NA.plot = TRUE)
ALE.11=ALEPlot(data[,-c(3,4,15)], gbm.data, pred.fun=yhat, J=11, K=100,
NA.plot = TRUE)
...
```{r ale 2D link, fig.width=7, fig.height=14}
# # To generate the code, uncomment the following lines.
# # But it is very slow because it calculates ALE for all variables,
# # so this vignette loads a pre-created model object.
# gbm_ale_2D_link <- ALE(
#   gbm.data,
#   x_cols = list(d2 = TRUE),  # All 2D interactions
#   data = data,
#   pred_fun = yhat,
#   max_num_bins = 100,
#   p_values = NULL
...
# Create plots object
gbm_link_2D_plots <- plot(gbm_ale_2D_link)

# Only print plots involving age
get(gbm_link_2D_plots, list(d2_all = 'age')) |> 
  print(ncol = 2)
```
...
```{r ale age x education_num}
# We can use the formula syntax to specify which interaction plot to retrieve
get(gbm_link_2D_plots, ~ age:education_num)
```
...
```{r ale 2D prob, fig.width=7, fig.height=10}
# # To generate the code, uncomment the following lines.
# # But it is slow because it calculates ALE for all variables,
# # so this vignette loads a pre-created model object.
# gbm_ale_2D_prob <- ALE(
...
# Create plots object
gbm_prob_2D_plots <- plot(gbm_ale_2D_link)

# Only print plots involving age
get(gbm_prob_2D_plots, list(d2_all = 'age')) |> 
  print(ncol = 2)
```
