...
```{r lm_simple, eval=FALSE, echo=TRUE}
...
```{r lm_simple-rds, eval=TRUE, echo=FALSE}
...
```{r lm_simple-plot, fig.width=7, fig.height=6}
# Print all plots
...
The constructor for `{S7}` `ModelBoot` object requires a model object as its first argument--any R model object that can generate numeric predictions. The second argument is the dataset. For objects that follow base R modelling conventions, `ModelBoot()` should be able to automatically recognize and parse the model object, so the data object is often optional. So, here is the creation of a `ModelBoot` object:

```{r lm_full_call, eval=FALSE, echo=TRUE}
# # To run the slow code yourself, uncomment and execute this code block directly.
# # For models like lm that store their data,
# # there is no need to specify the data argument.
...
```{r lm_full_call-rds, eval=TRUE, echo=FALSE}
...
```{r gam_simple, eval=FALSE, echo=TRUE, fig.width=7, fig.height=6}
# # To run the slow code yourself, uncomment and execute this code block directly.
...
```{r gam_simple-rds, eval=TRUE, echo=FALSE, fig.width=7, fig.height=6}
...
```{r gam_full_stats, eval=FALSE, echo=TRUE}
# # To run the slow code yourself, uncomment and execute this code block directly.
# # For models like mgcv::gam that store their data,
# # there is no need to specify the data argument.
...
```{r gam_full_stats-rds, eval=TRUE, echo=FALSE}
...
So, here is the form for constructing a `ModelBoot` for a model object type that is not automatically detected:

```{r model_call_string, eval=FALSE, echo=TRUE}
...
```{r model_call_string-rds, eval=TRUE, echo=FALSE}

mb_gam_attitude_special <- retrieve_rds(
  # For speed, load a pre-created object by default.
  c(serialized_objects_site, 'mb_gam_attitude_special.0.5.2.rds'),
  {
    # To run the code yourself, execute this code block directly.
    # For models like mgcv::gam that store their data,
    # there is no need to specify the data argument.
...
# saveRDS(mb_gam_attitude_special, file.choose())
