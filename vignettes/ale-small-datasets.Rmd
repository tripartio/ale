...
At the very least, the `{ale}` package is useful for visualizing the effects of model variables.
...
# Print all plots
lm_attitude_simple_plots <- plot(ale_lm_attitude_simple)
lm_attitude_simple_1D_plots <- lm_attitude_simple_plots@distinct$rating$plots$d1
patchwork::wrap_plots(lm_attitude_simple_1D_plots, ncol = 2)
```
...
We can also use `ALE()` to visualize the possible existence of any interactions by specifying `x_cols = list(d2 = TRUE)` to calculate all 2D interactions:

```{r lm_2D, fig.width=7, fig.height=7}
ale_lm_attitude_2D <- ALE(
  lm_attitude,
  x_cols = list(d2 = TRUE),
  # complete_d = 2,
  parallel = 2  # CRAN limit (delete this line on your own computer)
...
# Create an ALEPlots object
attitude_plots <- plot(ale_lm_attitude_2D)
attitude_2D_plots <- attitude_plots@distinct$rating$plots$d2
# Print plots
attitude_2D_plots |>
...
The constructor for `ModelBoot` has two required arguments. Consistent with tidyverse conventions, its first argument is a dataset, `data.` The second argument is the model object to be analyzed. For objects that follow standard R modelling conventions, `ModelBoot()` should be able to automatically recognize and parse the model object. So, here is the creation of a `ModelBoot` object:
...
```{r lm_full_ale, fig.width=7, fig.height=7}
mb_lm_plots <- plot(mb_lm)
mb_lm_1D_plots <- mb_lm_plots@distinct$rating$plots$d1
patchwork::wrap_plots(mb_lm_1D_plots, ncol = 2)
```
...
```{r gam_simple, fig.width=7, fig.height=7}
ale_gam_attitude_simple <- ALE(
  gam_attitude,
  parallel = 2  # CRAN limit (delete this line on your own computer)
)

gam_attitude_simple_plots <- plot(ale_gam_attitude_simple)
gam_attitude_simple_1D_plots <- gam_attitude_simple_plots@distinct$rating$plots$d1
patchwork::wrap_plots(gam_attitude_simple_1D_plots, ncol = 2)
```
...
```{r gam_full_ale, fig.width=7, fig.height=7}
