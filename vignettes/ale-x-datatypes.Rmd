...
vignette: >
  %\VignetteIndexEntry{ale package handling of various input datatypes}
...
This vignette demonstrates how `{ale}` works for various input datatypes. You should first read the [introductory vignette](ale-intro.html "Introduction to the ale package") that explains general functionality of the package; this vignette is a demonstration of specific functionality.
...
```{r ale_cars_1D, eval=FALSE, echo=TRUE, fig.width=7, fig.height=14}
...
```{r ale_cars_1D-rds, eval=TRUE, echo=FALSE, fig.width=7, fig.height=14}
ale_cars_1D <- retrieve_rds(
  # For speed, load a pre-created object by default.
  c(serialized_objects_site, 'ale_cars_1D.0.5.2.rds'),
  {
...
```{r ale_cars_2D, eval=FALSE, echo=TRUE, fig.width=7, fig.height=28}
# # # To run the slow code yourself, uncomment and execute this code block directly.
# # For models like mgcv::gam that store their data,
# # there is no need to specify the data argument.
# ale_cars_2D <- ALE(
...
```{r ale_cars_2D-rds, eval=TRUE, echo=FALSE, fig.width=7, fig.height=28}
...
There are no interactions in this model but the point of this demonstration is to show that the `ale` package can handle 2D interactions between just about any pair of interaction types: numeric-numeric, ordinal-binary, categorical-ordinal, etc.
...
```{r cars_full, eval=FALSE, echo=TRUE, fig.width=7, fig.height=14}
# # To run the slow code yourself, uncomment and execute this code block directly.
# # For models like mgcv::gam that store their data,
# # there is no need to specify the data argument.
...
```{r cars_full-rds, eval=TRUE, echo=FALSE, fig.width=7, fig.height=14}
