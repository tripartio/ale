...
#' @param rep instructions for calculating random effect probabilities (REPs) and to determine the median band. If `NULL` (default), no REPs are calculated and `median_band_pct` is used to determine the median band. To calculate REPs, an object generated by the [create_rep_dist()] function must be provided here. If `rep` is set to 'auto', this `ale()` function will try to automatically create the REPs function; this only works with standard R model types. Any error message will be given if REPs cannot be generated. Any other input provided to this argument will result in an error. For more details about creating REPs, see documentation for [create_rep_dist()]. Note that REPs will not be generated if 'stats' are not included as an option in the `output` argument.
...
#' default values used if `rep` are provided. If `rep` are not provided,
...
#' for printing bands around the median for single-variable plots. These are the
#' default values used if `rep` are not provided. If `rep` are provided,
...
#'       the provided `rep` object. `q` means that quantiles were calculated
#'       based on `median_band_pct` because `rep` was not provided.
...
# {
  ...
  if (!is.null(rep)) {
    # The user wants REPs
    if (length(rep) == 1 && rep == 'auto') {
      # Try to automatically obtain REPs
    ...
    else {  # an ale_rep object should be provided
      validate(
      ...
      .f = \(it.x_cols) {
        ...
        ale_results |>
          imap(\(it.cat_ar, it.cat_name) {
            ...
            #   ale_d = length(it.x_cols),  # dimension of ALE (1D or 2D)
            #   ale = list(it.cat_ar$summary),
            #   boot_data = list(it.cat_ar$boot_ale_y),
            #   stats = list(it.cat_ar$stats),
            # )
          }) |>
          bind_rows()
      }) |>   # furrr::future_map(
  ...
  for (it.cat in y_cats) {
    ...
    # ale_struc$distinct[[it.cat]]$ixn <- it.ar_2D |>
    #   map(\(it.el) {  # iterate result elements (ale, stats, etc.)
    #     it.el |>
    #       imap(\(it.el_tbl, it.2D_x_cols) {
    #         # Convert x_col names back into character(2) vector
    #         it.2D_x_cols <- it.2D_x_cols |>
    #           stringr::str_split('\\|') |>
    #           unlist()
    #
    #         # Return the tibble result embedded two levels down by the 2D x_cols
    #         it.return <- list()
    #         it.return[[it.2D_x_cols[1]]][[it.2D_x_cols[2]]] <- it.el_tbl
    #
    #         it.return
    #       }) # |>
  ...
  #     }
  #   }) |>
  #   # Transpose to group all elements by y category
  #   list_transpose(simplify = FALSE)
  # # }  # if (!ixn)


  # if (length(x_cols[[1]]) != 0) {
  #   ales_1D <-
  #     x_cols[[1]] |>
  #     # x_cols |>
  #     map(  # for debugging
  ...
  #
  #   # Transpose ales_2D to group elements together
  #   ales_2D <- ales_2D |>
  #     map(\(it.cat) {
  #       it.cat |>
  #         map(\(it.x1) {
  ...
  if ('stats' %in% output) {
    for (it.cat in y_cats) {
      ...
      # if (!is.null(ales_1D)) {
        ...
        # if ('plots' %in% output) {
        #   ales_1D[[it.cat]]$stats$effects_plot <- plot_effects(
        #     ales_1D[[it.cat]]$stats$estimate,
        #     y_summary[, it.cat],
        #     y_col,
      ...
      # for (it.cat in y_cats) {
        ...
        for (it.x1 in names(ale_struc$distinct[[it.cat]][[2]]$stats)) {
          ...
          # # Create an effects plot only if plots are requested
          # if ('plots' %in% output) {
          #   ales_2D[[it.cat]]$stats[[it.x1]]$effects_plot <- plot_effects(
          #     ales_2D[[it.cat]]$stats[[it.x1]]$estimate,
          #     y_summary[, it.cat, drop = FALSE],
          #     y_col,
          #     middle_band = if (is.null(rep)) {
          #       median_band_pct
