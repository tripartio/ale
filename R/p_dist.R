## p_dist.R
#
# Functions related to creating p-values for ALE statistics
#
...
#'
#' What we have just described is the precise approach to calculating p-values with the argument `p_speed = 'precise slow'`. Because it is so slow, by default, `create_p_dist()` implements an approximate algorithm by default (`p_speed = 'approx fast'`) which trains only a few random variables up to the number of physical parallel processing threads available, with a minimum of four. To increase speed, the random variable uses only 10 ALE bins instead of the default 100. Although approximate p-values are much faster than precise ones, they are still somewhat slow: at the very quickest, they take at least the amount of time that it would take to train the original model two or three times. See the "Parallel processing" section below for more details on the speed of computation.
#'
...
#'
#' For exact p-values, by default 1000 random variables are trained. So, even with parallel processing, the procedure is very slow. However, an `ale_p` object trained with a specific model on a specific dataset can be reused as often as needed for the identical model-dataset pair.
#'
...
#'
#' @return
#' The return value is an object of class `ale_p`. See examples for an illustration of how to inspect this list. Its elements are:
