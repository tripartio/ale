...
#' See details.
#' @param rep instructions for calculating random effect probabilities (REPs) and to determine the median band. If `NULL` (default), no REPs are calculated and
#' `median_band_pct` is used to determine the median band.
#' To calculate REPs, an object generated by the
#' [create_rep_dist()] function must be provided here. If `rep` is set to 'auto',
#' this `ale()` function will try to automatically create the REPs function;
#' this only works with standard R model types. Any error message will be given
#' if REPs cannot be generated. Any other input provided to this argument
#' will result in an error. For more details about creating REPs,
#' see documentation for [create_rep_dist()]. Note that REPs will not be
