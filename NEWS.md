...
* Along with parallelization, we have switched to using the `{progressr}` package for progress bars. with the `cli` progression handler, this enables accurate estimated times of arrival (ETA) for long procedures, even with parallel computing. Although they do not increase performance (on the contrary, they slow things down slightly), progress bars with ETA provide invaluable feedback that makes the wait for long procedures much more bearable. Unfortunately, because of the idiosyncrasies of the `{progressr}` package, users must explicitly enable progress bars once per session (though a permanent setting can be set in the users .Rprofile file). For details, see [help(ale)], particularly the documentation on progress bars and the `silent` argument.
